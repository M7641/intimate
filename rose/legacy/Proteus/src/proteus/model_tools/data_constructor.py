...
from .s3_tools import download_most_recent_tar_file, upload_to_s3
...
class DataConstructor:
    ...
    def __init__(
        self,
        data_config: DataStructureSingular,
        save_append_name: str = "",
    ) -> None:
    ...
    def process_data_for_model(
        ...
        ------
            None.
        """
        if self.data_config.data_structure_type == "single_row_relational":
            processed_data = self._process_single_row_relational(
                data=data,
                train_encoders=train_encoders,
            )
        elif self.data_config.data_structure_type == "event_as_array":
            processed_data = self._process_event_as_array(
                input_data=data,
                train_encoders=train_encoders,
            )
    ...
    def convert_rows_to_array(self, data: pd.DataFrame) -> pd.DataFrame:
        """Converts the rows of a pandas DataFrame into an array format.
        ...
        Returns
        -------
            pd.DataFrame: The converted DataFrame with each row represented as an array.
        """
        data_array = data.apply(lambda row: row.to_numpy(), axis=1)
        return pd.DataFrame(data_array, columns=[self.data_config.data_structure_name])

    def _save_to_s3(self, file, object_name: str):
        ...
        upload_to_s3(
            file=file,
            model_name=self.data_config.data_structure_name + self.save_append_name,
    ...
    def _load_from_s3(
        ...
        return joblib.load(load_from + f"{object_name}.joblib")
    ...
    def _save_pickle(
        ...
        """Save the encoder to S3.

        Parameters
        ----------
            encoder (Encoder): The encoder object to be saved.
            encoder_name (str): The name of the encoder.
        ...
        -------
            None
        """
        if self.data_config.encoder_save_mode == "s3":
            self._save_to_s3(file, object_name)

    def _load_pickle(
        self,
        object_name: str,
    ):
        """Load a pickle file containing an encoder based on the specified encoder name.
        ...
        """
        if self.data_config.encoder_save_mode == "s3":
            # TODO: Work out what happens when the encoder is not found
            # Raise an error to say it's not there and have they ran train first.
            encoder = self._load_from_s3(object_name)
    ...
    def infer_low_cardinality_categorical_columns(
        ...
        low_cardinality = cardinality.loc[
            cardinality["cardinality"] <= self.data_config.ordinal_encoder_cut_off_point
        ]["index"].to_list()
    ...
    def infer_high_cardinality_categorical_columns(
        ...
        if cardinality["cardinality"].any() > 100:
    ...
    def _process_single_row_relational(
        ...
        if self.data_config.manual_encoding_choices is not None:
            ...
            self._save_pickle(
                file=inferred_encodings,
                object_name="infered_encodings_save",
            )
        ...
        previous_columns = data.columns.to_list()
    ...
    def _process_event_as_array(
        ...
        processed_data.index.names = [self.data_config.entity_id]
        processed_data = processed_data.reset_index()
        processed_data = processed_data.sort_values(
            self.data_config.event_as_array_date_column,
        )

        processed_data = processed_data.groupby([self.data_config.entity_id]).head(
            self.data_config.event_as_array_sequence_length,
        )

        packaged_arrays = {}

        for group_data in processed_data.groupby(self.data_config.entity_id):
            padded_array = np.zeros(
                (self.data_config.event_as_array_sequence_length, total_features),
            )
            array_to_pad = (
                group_data[1].drop(columns=[self.data_config.entity_id]).to_numpy()
            )
            padded_array[
                : array_to_pad.shape[0],
                : array_to_pad.shape[1],
            ] = array_to_pad
            packaged_arrays[group_data[0]] = padded_array.tolist()

        packaged_data = pd.DataFrame.from_dict(packaged_arrays, orient="index")
        packaged_data.index.name = self.data_config.entity_id

        # some situations had group_data[0] as a str, and others a list which breaks.
        # this removes either permuation being an issue, but we need clarity on why this happens.
        if not isinstance(packaged_data.index.to_numpy()[0], str):
            vfunc = np.vectorize(lambda x: x[0])
            packaged_data.index = vfunc(packaged_data.index.to_numpy())
