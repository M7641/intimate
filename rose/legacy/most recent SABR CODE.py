...
def calibration(starting_par,F,K,time,MKT):
    for i in range(len(F)):
        x0 = starting_par
        bnds = ( (0.001,None) , (0.5,0.5) , (-0.999,0.999) , (0.001,None)  )
...
outvol = open('D:\py\outvol.csv', 'w')             # file output of volatilities
vol_diff = open('D:\py\differences.csv', 'w')  # file output differences between SABR and Market volatilities
parameters = open('D:\py\parameters.csv', 'w')     # file output parameters

        
workbook = xlrd.open_workbook('D:\py\market_data.xlsx')
Market_data = workbook.sheet_by_name('Swaptions data')
...
regr.fit(X,Y)
ypre = regr.predict(X)
alphapre = regr.predict(0)
print(alphapre)
# The coefficients
print('Coefficients: \n', regr.coef_)
# Plot outputs
plt.scatter(X,Y,  color='black')
plt.plot(X, ypre, color='red', linewidth=3)
