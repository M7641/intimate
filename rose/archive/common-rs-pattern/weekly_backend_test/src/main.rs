...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    init_tracing("info,weekly_backend=debug,sqlx::query=error");

    let env = EnvManager::from_env();
    let cfg = RedshiftConfig::from_env()?;
    tracing::info!(target = env.target().as_str(), "starting weekly-backend");
    ...
    let dist = DistPath::resolve(
        PathBuf::from(env!("CARGO_MANIFEST_DIR"))
            .parent()
            .expect("backend has parent (weekly module dir)")
            .join("frontend")
            .join("dist"),
    );
    ...
    tracing::info!(%addr, "weekly-backend listening");
