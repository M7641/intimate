...
A gold frame is just an input frame (``id`` + ``description``) plus extra
columns holding the *expected* value for each schema feature. Cells
...
def score_frame(
    ...
    *,
    ...
    """
    kinds = {f.name: f.kind for f in flatten(schema)}
    ...
    for field in schema.column_names():
        ...
        for grow in gold.iter_rows(named=True):
            ...
            if match(kind, gold_val, prediction):
                correct += 1
        if support:
...
def evaluate(
    ...
    *,
    ...
    pred = extract_features(inputs, schema, extractor, description_col=description_col)
    return pred, score_frame(pred, gold, schema, key=key)


def flatten(schema: ExtractionSchema) -> list[ExtractionField]:
...
def match(kind: FieldKind, gold: object, pred: object) -> bool:
    if pred is None:
        return False
    ...
    return norm(gold) == norm(pred)  # text / categorical
