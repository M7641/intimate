...
from .images import deploy_image
from .log import NimbusLogger
...
class Workflows:
    ...
    def create_or_update_workflow(self, spec: dict) -> dict:
        ...
        existing_workflow = next(
            (w for w in workflows if w["name"] == spec["name"]), None
        )
    ...
    workflow_image_id, _ = deploy_image(
        body={
            ...
            "buildDetails": {
                ...
                # Nimbus's image API requires an array here; an empty dict {} is
                # rejected as null. build_args, when given, is a list of
                # {"name": ..., "value": ...} entries.
