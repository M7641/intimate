...
instructions sent to the model, validate/normalise the JSON output, and enumerate
...
class ExtractionSchema:
    ...
    def prompt_block(self) -> str:
        ...
        for f in self.fields:
            lines.append(f"  - {describe_field(f)}")
        conditional = [
    ...
    def json_schema(self) -> dict:
        ...
        for f in self.fields:
            properties[f.name] = field_json_schema(f)
            for subs in f.children.values():
                for child in subs:
                    properties.setdefault(child.name, field_json_schema(child))
        return {
            "type": "object",
    ...
    def validate(self, raw: dict) -> dict:
        ...
        for f in self.fields:
            ...
            for child in subs or ():
                out[child.name] = coerce(child, raw.get(child.name))
        return out
...
def field_from_dict(d: dict) -> ExtractionField:
    children = {
        value: tuple(field_from_dict(c) for c in subs)
...
def field_json_schema(f: ExtractionField) -> dict:
    spec: dict = {"type": [JSON_TYPE[f.kind], "null"]}
    if f.kind is FieldKind.CATEGORICAL:
        spec["enum"] = [*f.choices, None]
    return spec


def describe_field(f: ExtractionField) -> str:
    head = f'"{f.name}"'
    if f.kind is FieldKind.CATEGORICAL:
...
def parse_number(value: object) -> float | None:
    ...
    if isinstance(value, (int, float)):
        return float(value)
    match = NUMERIC.search(str(value).replace(",", ""))
    return float(match.group()) if match else None


def coerce(f: ExtractionField, value: object) -> object:
