//! CLI unifiée de snowhouse : serveur, dev local et déploiement.
//!
...
#[command(
    name = "snowhouse",
    about = "snowhouse — analytics app: serve, local dev, deploy.",
    version
)]
pub struct Cli {
    #[command(subcommand)]
...
pub enum CliCommand {
    ...
    Deploy,
    /// Manage the persistent query-history cache table.
    Cache {
        #[command(subcommand)]
        action: CacheAction,
    },
...
/// Lifecycle operations on the query-history cache (see `src/cache/`).
#[derive(Subcommand, Clone)]
pub enum CacheAction {
    /// Create the cache table if it does not already exist.
    Init,
    /// Backfill the cache from information_schema (clamped to the 7-day retention).
    Backfill {
        /// Days of history to sweep in (1–7).
        #[arg(long, default_value_t = 7)]
        days: u32,
    },
    /// Top up the cache with queries since the last cached one (hourly job).
    Refresh,
    /// Print cache stats: row count, covered time span, last ingest.
    Status,
}
...
pub fn dispatch(command: CliCommand) -> i32 {
    match command {
        // `serve` et `cache` démarrent tokio ; ils sont traités dans `main`.
        CliCommand::Serve => unreachable!("serve is handled in main"),
        CliCommand::Cache { .. } => unreachable!("cache is handled in main"),
...
/// `cargo build -p snowhouse` trouve sa racine. `CARGO_MANIFEST_DIR` vaut
/// `cocoon/apps/snowhouse` → `../..` = `cocoon`.
...
fn start(install_first: bool, reload: bool) -> i32 {
    ...
    // Pre-flight: refuse to start if the API port is already taken. Otherwise the
    // backend we spawn panics with AddrInUse while the *stale* server keeps
    // answering :8050 — and the readiness probe, unable to tell the two apart,
    // would greenlight the dev dashboard against the wrong API. Fail fast, and do
    // NOT start the frontend.
    if port_in_use(API_PORT) {
        cli_ui::error(&format!(
            "port {API_PORT} is already in use — another snowhouse backend is \
             probably running. Stop it and retry; the dev dashboard was not started."
        ));
        return 1;
    }
    ...
    // Snowflake-only: the connector is the default feature, no selection needed.
...
/// one-shot `cargo run`. The Snowflake connector is the crate's default feature.
...
/// The TCP port the backend binds (`0.0.0.0:8050` in `main.rs`). The launcher
/// checks it is free before starting anything, then polls it for readiness.
const API_PORT: u16 = 8050;

/// Whether something is already listening on `port` (a successful TCP connect).
/// Used both as a pre-flight guard and as the readiness fallback.
fn port_in_use(port: u16) -> bool {
    std::net::TcpStream::connect(("127.0.0.1", port)).is_ok()
}

/// Whether the backend answers on the API port — an HTTP 200 from `/health/live`
/// if `curl` is available, else a raw TCP connect (the port is only bound once
/// the server is fully initialised, so a connection is a reliable readiness
/// signal — and the pre-flight guarantees the listener is ours, not a stale one).
fn backend_is_ready() -> bool {
    let health = format!("http://localhost:{API_PORT}/health/live");
    if command_succeeds("curl", &["-s", "-f", "-o", "/dev/null", health.as_str()]) {
        return true;
    }
...
/// Pre-deploy test gate: run the app's unit tests, returning their exit code.
///
/// A deploy ships an image to Nimbus, so we refuse to do it on a red suite. We use
/// `cargo nextest` — the repo's proto-pinned test runner — and fall back to
/// `cargo test` when nextest is not installed, mirroring the bacon fallback used
/// by the dev launcher above. Tests run with the crate's default features
/// (`snowflake`); `run()` enforces the source-checkout requirement for us.
fn run_predeploy_tests() -> i32 {
    cli_ui::note("running unit tests before deploy (aborts on any failure)…");
    if command_succeeds("cargo", &["nextest", "--version"]) {
        return run(&["cargo", "nextest", "run", "-p", APP], project_root());
    }
    cli_ui::note("cargo-nextest not found — falling back to `cargo test`.");
    run(&["cargo", "test", "-p", APP], project_root())
}
...
fn deploy() -> i32 {
    ...
    // Test gate: never ship a red build. Run the unit tests first and abort —
    // before touching the registry or Nimbus — if any fail.
    let test_code = run_predeploy_tests();
    if test_code != 0 {
        cli_ui::error("unit tests failed — aborting deploy (nothing was pushed).");
        return test_code;
    }
    cli_ui::success("unit tests passed — proceeding with deploy");
    ...
    // in `FromEnv` mode preserves this path's historical behaviour: the runtime
    // credential (API_KEY) is read from the environment and forwarded to the
    // build (the Dockerfile promotes it to a runtime env var). It is already
    // required — and validated — by `Client::from_env()` above.
    let opts = deploy_catalog::find(APP)
        .expect("snowhouse is registered in deploy-catalog")
        .to_deploy_service(
