...
import { useRedshiftInfo } from "@/hooks/useRedshiftData";
...
import { redshiftRoutes, type RouteConfig } from "@/routes";

function DatabaseBadge() {
  const { data: info, isLoading } = useRedshiftInfo();
  if (isLoading) {
      ...
      >
        ...
        {sidebarOpen && (
          <div className="border-b flex items-center gap-1 px-2 h-12">
            <FaDatabase className="text-muted-foreground shrink-0 ml-1" size={14} />
            <DatabaseBadge />
        ...
        <nav className="flex-1 py-3 overflow-y-auto pr-3 space-y-4">
          <NavSection
            label="Redshift Analytics"
            items={redshiftRoutes}
