...
/// Maps short aliases (`opus`, `sonnet`, `haiku`) to full model IDs.
/// Unknown names pass through unchanged, so full IDs still work.
...
impl LlmClient for ClaudeCliClient {
    async fn prompt_with_model(
        ...
        // Aliases: opus, sonnet, haiku — see `resolve_model`.
        // https://code.claude.com/docs/en/cli-reference
        let model = resolve_model(model.unwrap_or("sonnet"));
        let output = tokio::process::Command::new("claude")
            .args([
                "--bare", // skips "auto-discovery" of hooks, skills, plugins, mcp, ect.
                "-p",
                &full_prompt,
                "--output-format",
                "text",
...
impl LlmClient for RigAnthropicClient {
    async fn prompt_with_model(
        ...
        let agent = client
            .agent(resolve_model(model.unwrap_or("sonnet")))
            .preamble(system)
            .build();
